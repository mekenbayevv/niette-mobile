...
(function (root) {
  ...
  function renderProfit(k, pk, prg) {
    ...
    const buyer = k.delivery_buyer > 0
      ? '<p class="note">Покупатели доплатили за доставку ' + money(k.delivery_buyer) +
        ' — эти деньги получает Kaspi, расход продавца они не уменьшают. Старая вкладка их вычитала и завышала прибыль.</p>'
      : '';
  ...
  function renderNotes() {
    return '<ul class="notes">' +
      ...
      '<li><b>Доставка</b> — сколько платит продавец. Доплату покупателя получает Kaspi; старая вкладка вычитала её из расходов на доставку и завышала прибыль.</li>' +
